...
namespace pfAdapter
{
	...
	public class Setting
	{
		...
		public Client Client_GetExePermission = new Client();
		public List<Client_WriteStdin> ClientList_WriteStdin = new List<Client_WriteStdin>();		//データ送信先のプロセス
		public ClientList MidProcessList = new ClientList();																		//定期実行プロセス
		public ClientList PostProcessList = new ClientList();																		//終了時実行プロセス
		...
		public static Setting LoadFile(string xmlpath = null)
		{
			...
			if (string.IsNullOrEmpty(xmlpath) == false && File.Exists(xmlpath) == false)
			{
				...
				Log.System.WriteLine("XmlPath  :" + Args.XmlPath);
				return null;
			}
			...
			file.sArgumets = (string.IsNullOrWhiteSpace(file.sArgumets))
															? new String(' ', 8) : file.sArgumets;
		...
		public static Setting LoadSample_A()
		{
			...
			//Client_GetExePermission
			setting.Client_GetExePermission = new Client()
			{
			...
			setting.ClientList_WriteStdin = new List<Client_WriteStdin>() 
			{
				...
				new Client_WriteStdin()
				{
					...
					sBaseArgs = "   -stdin \"$fPath$\"   -o \"$fPath$.pp\"  -ia 4  -fo 0  -yr 2  -om 0  -hide  -exit   ",
					dDelay_sec = 3,
				},
				//create_lwi
				new Client_WriteStdin()
				{
					sBasePath = @"   ..\create_lwi\create_lwi13.exe   ",
		...
		public static Setting LoadSample_B()
		{
			...
			setting.ClientList_WriteStdin = new List<Client_WriteStdin>() 
			{
				...
				new Client_WriteStdin()
				{
					bEnable = 1,
					sBasePath = @"   ..\DGIndex_pf\DGIndex_pf.exe   ",
					sBaseArgs = "   -stdin \"$fPath$\"   -o \"$SubDir$\\$fName$.pp\"  -ia 4  -fo 0  -yr 2  -om 0  -hide  -exit   ",
					dDelay_sec = 3,
				},
				//create_lwi
				new Client_WriteStdin()
