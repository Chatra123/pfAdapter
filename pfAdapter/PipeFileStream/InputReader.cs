...
namespace pfAdapter
{
	...
	class InputReader
	{
		...
		public void SetParam(double newPipeBuff_MiB = -1, double newReadLimit_MiBsec = -1)
		{
			...
			//ログ
			//  pipe
			if (string.IsNullOrEmpty(iPipeName) == false)
			{
				if (pipeReader != null && pipeReader.IsConnected)
					Log.System.WriteLine("  Connect pipe        buffmax = {0,2:N0}    MiB", pipeReader.BuffMaxSize / 1024 / 1024);
				else
					Log.System.WriteLine("  Not found pipe server");
			}

			//  file
			if (fileReader != null)
				Log.System.WriteLine("  Create  fileReader    limit = {0,5:f2} MiB/sec", ReadSpeedLimit / 1024 / 1024);
		...
		public bool ConnectInput(string ipipe, string ifile)
		{
			...
			if (pipeReader == null && fileReader == null)
				if (string.IsNullOrEmpty(iFilePath) == false)				//ファイル名が指定されている
					for (int i = 0; i < 5 * 10; i++)									//まだファイルが作成されていない？
					{
						Thread.Sleep(200);
						if (File.Exists(iFilePath))
						{
		...
		public byte[] ReadBytes()
		{
			...
			if (pipeReader != null)
			{
				for (int retry = 0; retry <= 2; retry++)
				{
					...
					readData = pipeReader.Read(filePositon, Packet.Size * 3000, out Indicator_demandAtBuff);
			...
			if (fileReader != null)
			{
				for (int retry = 0; retry <= 2; retry++)
				{
					...
					readData = fileReader.ReadBytes(Packet.Size * 3000);			//Packet.Size * 3000 = 564 KiB
					...
					{																									//ゼロパケット＆最後尾の値パケットは必ず破棄される
						...
						if (removeZeroPacket)
						{
							Thread.Sleep(500);														//未書込みエリアを読み込んだのでSleep()
					...
					else
					{
						...
						if (retry < 2)
						{
							...
							//readDataの最後のbyteが０？
							var lastData = (int)readData[readData.Count() - 1];
							if (lastData == 0) Thread.Sleep(5 * 1000);		//未書込みでのゼロかも、追加待機
							continue;																			//リトライ  for
						}
					}
